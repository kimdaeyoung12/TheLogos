...
<div class="mle-ecr-visualization my-16 antialiased text-slate-700">
    <style>
        .mle-ecr-visualization .tab-active { border-bottom: 2px solid #2563eb; color: #2563eb !important; font-weight: 600; }
        .mle-ecr-visualization .tab-inactive { border-bottom: 2px solid transparent; color: #64748b; }
        .mle-ecr-visualization .tab-inactive:hover { color: #3b82f6; }
    ...
    <div class="max-w-5xl mx-auto bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
        ...
        <!-- Navigation -->
        <div class="flex border-b border-slate-100 px-8 pt-4 bg-slate-50">
            <button id="btn-mle-v2" class="px-8 py-4 tab-active bg-transparent text-sm md:text-base" onclick="switchTabV2('mle')">
                1. 최대 우도 추정 (MLE) - LLM
            </button>
            <button id="btn-ecr-v2" class="px-8 py-4 tab-inactive bg-transparent text-sm md:text-base" onclick="switchTabV2('ecr')">
                2. 기대 누적 보상 (ECR) - RL
            </button>
        </div>
        ...
        <div class="p-8 md:p-10">
            ...
            <div id="section-mle-v2" class="block">
                <div class="grid lg:grid-cols-2 gap-12 items-start">
                    <!-- Text Description -->
                    <div>
                        <h3 class="text-xl font-bold text-slate-800 mb-4">최대 우도 추정 (Maximum Likelihood Estimation)</h3>
                        ...
                        <div class="bg-blue-50/50 p-6 rounded-2xl border border-blue-100 mb-6">
                            <h4 class="font-bold text-blue-800 mb-2">실습: 확률 분포 맞추기</h4>
                            <p class="text-sm text-blue-700 mb-6">슬라이더를 움직여 파란색 곡선(모델의 가설)이 검은 점(실제 데이터)의 분포를 가장 잘 설명하도록 위치를 맞춰보세요.</p>
                            
                            <div class="px-2">
                                <label for="mle-slider-v2" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">모델 파라미터 (평균 $\mu$)</label>
                                <input type="range" id="mle-slider-v2" min="0" max="100" value="25" class="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600">
                        ...
                        <div class="p-5 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between">
                            ...
                            <span id="mle-score-v2" class="text-2xl font-black text-blue-600">Low</span>
                        </div>
                    </div>
                    
                    <!-- Graph -->
                    <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-inner h-[350px]">
            ...
            <div id="section-ecr-v2" class="hidden">
                <div class="grid lg:grid-cols-2 gap-12 items-start">
                    <!-- Text Description -->
                    <div>
                        <h3 class="text-xl font-bold text-slate-800 mb-4">기대 누적 보상 (Expected Cumulative Reward)</h3>
                        <p class="mb-4 text-slate-600 leading-relaxed">
                        ...
                        <div class="bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100 mb-6">
                            <h4 class="font-bold text-emerald-800 mb-2">실습: 전략 시뮬레이션</h4>
                            <p class="text-sm text-emerald-700 mb-6">당장 눈앞의 이익을 쫓는 '전략 A'와 초기 손실을 감수하더라도 최종 보상을 노리는 '전략 B'를 비교해보세요.</p>
                            
                            <div class="flex gap-3">
                                <button onclick="animateEcrV2('greedy')" class="flex-1 bg-white border border-slate-200 text-slate-700 py-3 px-4 rounded-xl hover:bg-slate-50 font-semibold text-sm">전략 A (단기 이익)</button>
                                <button onclick="animateEcrV2('optimal')" class="flex-1 bg-emerald-600 text-white py-3 px-4 rounded-xl hover:bg-emerald-700 font-semibold text-sm shadow-lg shadow-emerald-200">전략 B (장기 가치)</button>
                            </div>
                        </div>

                        <div class="p-5 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between">
                            <span class="text-sm font-semibold text-slate-500">Total Cumulative Return</span>
                            <span id="ecr-score-v2" class="text-2xl font-black text-emerald-600">0</span>
                        </div>
                    </div>
    ...
    <script>
        ...
        (function() {
            ...
            const mleChart = new Chart(ctxMle, {
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: { min: -5, max: 120, grid: { color: '#f1f5f9' } }
                    },
                    plugins: {
                        legend: { display: false }
                    },
            ...
            function updateMle() {
                ...
                if (dist < 3) { scoreDisp.innerText = "MAXIMUM!"; scoreDisp.className = "text-2xl font-black text-blue-600 animate-pulse"; }
                else if (dist < 15) { scoreDisp.innerText = "MEDIUM"; scoreDisp.className = "text-2xl font-black text-amber-500"; }
                else { scoreDisp.innerText = "LOW"; scoreDisp.className = "text-2xl font-black text-slate-400"; }
            }
            ...
            const ecrChart = new Chart(ctxEcr, {
                ...
                options: {
                    ...
                    maintainAspectRatio: false,
                    scales: {
                        y: { min: -10, max: 250, grid: { color: '#f1f5f9' } },
                        x: { grid: { display: false } }
                    },
