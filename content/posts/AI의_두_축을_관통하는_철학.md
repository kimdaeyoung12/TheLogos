...
<head>
    ...
    <title>AI의 두 축: RL과 LLM의 철학적 탐험</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Pretendard:wght@300;400;600;700&display=swap');
        body { font-family: 'Pretendard', sans-serif; background-color: #fdfcf9; color: #1e293b; scroll-behavior: smooth; }
...
<!-- Chosen Palette: Warm Minimalist (Background: #fdfcf9, Accent: Indigo/Violet for LLM, Emerald for RL) -->
<!-- Application Structure Plan: 
    1. Hero Section: AI 학습의 두 가지 철학적 패러다임(연역/귀납) 소개.
    2. Interactive Comparison Matrix: RL과 LLM의 핵심 차이점을 목적, 학습방식, 데이터 관점에서 대조.
    3. The Deductive Explorer (RL): AWS 리포트 기반, 보상 루프와 강화학습의 메커니즘을 시각화 및 보상 성장 차트 제공.
    4. The Inductive Scholar (LLM): 카카오페이 리포트 기반, 6단계 파이프라인을 인터랙티브 스테퍼로 구성.
    5. The Synthesis (RLHF): 두 철학이 만나는 지점을 설명하며 마무리.
    사용자는 상단 네비게이션을 통해 비선형적으로 탐색 가능하며, 각 섹션마다 데이터 시각화를 통해 개념을 직관적으로 이해함. -->

<!-- Visualization & Content Choices:
    - AI Philosophy Mapping: HTML Grid + JS Toggle. 추상적 개념을 구체적 대비로 표현.
    - RL Reward Growth: Chart.js (Line). 시간에 따른 누적 보상 최적화 과정을 시각화.
    - LLM Token Probability: Chart.js (Bar). 다음 토큰 예측의 확률적 본질을 표현.
    - RL Loop Diagram: Structured HTML/CSS. 에이전트-환경 상호작용을 이미지 없이 구현.
    - LLM 6-Step Pipeline: Interaction-driven Stepper. 복잡한 단계를 논리적 순서로 분해.
    - CONFIRMATION: NO SVG graphics used. NO Mermaid JS used. -->

<nav class="fixed top-0 w-full z-50 glass shadow-sm px-6 py-4">
    <div class="max-w-6xl mx-auto flex justify-between items-center">
        <h1 class="font-bold text-xl tracking-tight text-indigo-900">AI Philosophy Explorer</h1>
...
<main class="mt-24 pb-20 px-6">
    <!-- Hero Section -->
    <section id="philosophy" class="max-w-4xl mx-auto py-16 text-center">
        <h2 class="text-4xl font-extrabold mb-6 leading-tight">AI의 두 축: <br><span class="text-emerald-600">연역적 탐험</span>과 <span class="text-indigo-600">귀납적 통찰</span></h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
            <div class="p-8 rounded-2xl glass border-emerald-100 card-hover">
                <div class="flex items-center mb-4">
                    ...
                    <h3 class="text-xl font-bold text-emerald-800">연역적 탐험가 (Deductive)</h3>
                </div>
            ...
            <div class="p-8 rounded-2xl glass border-indigo-100 card-hover">
                <div class="flex items-center mb-4">
                    ...
                    <h3 class="text-xl font-bold text-indigo-800">귀납적 학자 (Inductive)</h3>
                </div>
    ...
    <!-- RL Section -->
    <section id="rl-section" class="max-w-5xl mx-auto py-16 border-t border-slate-100">
        <div class="flex flex-col md:flex-row gap-12 items-center">
            <div class="flex-1">
                <span class="text-emerald-600 font-bold tracking-widest text-xs uppercase">Reinforcement Learning</span>
                <h2 class="text-3xl font-bold mt-2 mb-6 text-slate-800">보상으로 배우는 지능</h2>
                ...
                <div class="bg-emerald-50 p-6 rounded-xl border border-emerald-100">
                    <h4 class="font-bold text-emerald-800 mb-3 text-sm">학습의 핵심: 정책 파라미터 자동 갱신</h4>
            ...
            <div class="flex-1 w-full bg-white p-6 rounded-2xl shadow-xl border border-slate-50">
                <h4 class="text-center font-bold text-slate-700 mb-4 text-sm">에피소드 반복에 따른 누적 보상 최적화</h4>
                <div class="chart-container">
                    <canvas id="rlChart"></canvas>
                </div>
                <div class="mt-6 flex justify-center space-x-4">
                    <button onclick="updateRLData()" class="bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-emerald-700 transition-colors">에피소드 시뮬레이션</button>
                    <button onclick="resetRLData()" class="bg-slate-100 text-slate-600 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-slate-200">초기화</button>
                </div>
            </div>
        </div>
    </section>
    ...
    <!-- LLM Section -->
    <section id="llm-section" class="max-w-5xl mx-auto py-16 border-t border-slate-100">
        <div class="text-center mb-12">
            <span class="text-indigo-600 font-bold tracking-widest text-xs uppercase">Large Language Model</span>
            <h2 class="text-3xl font-bold mt-2 mb-4 text-slate-800">텍스트 우주에서의 귀납적 추론</h2>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-1 space-y-4">
                <div id="step-btn-1" onclick="showStep(1)" class="p-4 rounded-xl glass border-indigo-200 cursor-pointer hover:bg-indigo-50 transition-all step-btn active-step">
                    <h4 class="font-bold text-indigo-800">Step 1: Tokenization</h4>
                    <p class="text-xs text-slate-500 mt-1">문장을 의미 단위(토큰)로 분해</p>
                </div>
                <div id="step-btn-2" onclick="showStep(2)" class="p-4 rounded-xl glass border-slate-100 cursor-pointer hover:bg-indigo-50 transition-all step-btn">
                    <h4 class="font-bold text-slate-700">Step 2: Embedding</h4>
                    <p class="text-xs text-slate-500 mt-1">토큰을 고차원 벡터 좌표로 변환</p>
                </div>
                <div id="step-btn-3" onclick="showStep(3)" class="p-4 rounded-xl glass border-slate-100 cursor-pointer hover:bg-indigo-50 transition-all step-btn">
                    <h4 class="font-bold text-slate-700">Step 3: Attention</h4>
                    <p class="text-xs text-slate-500 mt-1">단어 간의 문맥적 관계 가중치 계산</p>
                </div>
                <div id="step-btn-4" onclick="showStep(4)" class="p-4 rounded-xl glass border-slate-100 cursor-pointer hover:bg-indigo-50 transition-all step-btn">
                    <h4 class="font-bold text-slate-700">Step 4: Prediction</h4>
                    <p class="text-xs text-slate-500 mt-1">다음 토큰의 확률 분포 생성</p>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 bg-slate-900 rounded-3xl p-8 text-white min-h-[400px] flex flex-col justify-center relative overflow-hidden">
                <div id="step-content" class="relative z-10">
                    <!-- Dynamic Content -->
                    <h3 class="text-2xl font-bold text-indigo-400 mb-4" id="step-title">Tokenization</h3>
    ...
    <section id="synthesis" class="max-w-4xl mx-auto py-16 text-center">
        <div class="bg-gradient-to-br from-indigo-900 to-emerald-900 rounded-[3rem] p-12 text-white shadow-2xl relative overflow-hidden">
            <h2 class="text-3xl font-bold mb-6">융합의 혁명: RLHF</h2>
            ...
            <div class="flex flex-col md:flex-row justify-center gap-4">
                ...
                <div class="px-2 py-3 text-2xl font-light opacity-50">=</div>
                <div class="px-6 py-3 bg-indigo-500 rounded-full shadow-lg text-sm font-bold">인간 친화적 AI</div>
            </div>
        </div>
    </section>
</main>
...
<footer class="py-12 border-t border-slate-100 text-center">
    <p class="text-slate-400 text-sm">© 2026 AI Philosophy & Tech Reports Explorer. Built for understanding.</p>
</footer>
    ...
    window.onload = () => {
        ...
        window.addEventListener('scroll', () => {
            ...
            sections.forEach(section => {
                ...
                if (window.pageYOffset >= (element.offsetTop - 150)) {
                    current = section;
                }
            });
