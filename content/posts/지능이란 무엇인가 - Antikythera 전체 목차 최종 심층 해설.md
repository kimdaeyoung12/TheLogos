...
Antikythera 프로젝트는 **‘지능(intelligence)’을 인간 중심적 관점에서 벗어나**, 생명, 물질, 기계, 그리고 우주 전반의 **정보 처리와 자기 조직화 현상**으로 확장하여 탐구한다. 이 문서는 공식 웹사이트([whatisintelligence.antikythera.org](https://whatisintelligence.antikythera.org))의 전체 목차를 기반으로 각 장의 철학적, 과학적 핵심을 2단계 깊이로 정리한 것이다.
...
* 지능은 세계의 패턴을 압축해 **간결한 모델**을 만드는 능력.
...
Antikythera는 지능을 단일한 정의로 고정하지 않는다. 대신, **물리적 법칙 → 생명 → 의식 → 기계 → 문명**으로 이어지는 거대한 연속체로 이해한다.

지능은 단순히 계산이 아니라, **질서와 의미가 생성되는 우주의 자기 반사 과정**이다.
