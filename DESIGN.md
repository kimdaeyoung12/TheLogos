...
Live Prayer의 Presence는 홈 화면과 같은 따뜻한 빛·타원 궤도·익명 입자 언어를 사용하되 기도문보다 낮은 대비로 한 단계 뒤에 둔다. 각 지체는 평면 점이 아니라 따뜻한 중심광, 얇은 색광, 넓은 Halo가 겹친 익명 빛으로 표현한다. 1명도 즉시 알아볼 수 있어야 하며, 50명 이상에서는 현재 렌더된 단계명·말씀·기도문·타이머의 바깥 경계를 따라 밝은 중심광을 재배치하고 밀도에 따라 크기를 줄여 하나의 고요한 군집을 이룬다. 낮은 대비의 Halo와 연결선은 콘텐츠 뒤를 지날 수 있지만 밝은 중심광은 글자 획과 겹치지 않는다. 정확한 지체 수는 같은 장면 안의 작은 caption으로 제공한다. 참여자 공지는 장면과 분리된 compact notice이며, 참여자가 닫을 수 없다. 새 공지로 교체되거나 Admin이 명시적으로 내릴 때까지 모든 일반 화면과 Focus 화면에서 유지한다.
...
- Retreat prayer constraints: static Hugo/GitHub Pages frontend plus a dedicated Supabase project; private Presence through invisible anonymous Auth; RLS and column grants are the data boundary; anonymous writes and owner-only account changes go through Edge Functions; Admin and participant auth storage are isolated; no service-role secret is shipped to the browser. Live/content subscriptions install idempotently, reconcile both snapshots after connection, and retry initial failure with a capped 2/5/15/30-second backoff. Durable announcements have a server-issued identity and timestamp, and only the current Live Controller can publish or clear them against the latest live-session version. The live 3D backdrop samples at most 72 anonymous light points, uses deterministic text-safe orbital spacing, and redraws at no more than 15fps/DPR 1.25, while the text count always shows the unsampled active-browser-connection value. Focus content may remain wheel/touch scrollable for unusually long copy but must not expose an inner scrollbar track. Validate unit tests, JS module graph, Edge syntax, Hugo build, 360/390/1280 layouts, 200% zoom and mobile landscape, controller generation/version races, and staging RLS before production activation.
