...
- 50명이 한 시각에 페이지를 열어도 말씀·기도제목은 먼저 보이지만, 현재 분산 간격을 합산하면 전체 Presence 인원 수가 안정되는 데 이론상 최대 약 48초가 걸릴 수 있다. 이는 아직 50개 고유 JWT로 확인하지 않은 보수적 예상값이며, 이 구간을 연결 실패로 오인해 새로고침을 반복하지 않도록 운영 전에 안내한다.
